  ...
  <BaseModal :dismissible="!pending" @close="$emit('close')">
    ...
    <form @submit.prevent="submit">
      ...
      <div class="modal-actions">
        <button v-if="task" type="button" class="danger-btn" :disabled="pending" @click="$emit('delete')">Delete task</button><span v-else></span>
        <div class="modal-right"><button type="button" class="ghost-btn" :disabled="pending" @click="$emit('close')">Cancel</button><button class="primary-btn" type="submit" :disabled="pending">{{ pending ? 'Saving…' : task ? 'Save changes' : 'Add task' }}</button></div>
      </div>
    </form>
  </BaseModal>
</template>
